# Deploying to Fly.io (paid alternative)

This is a paid path (a few USD/month, no lasting free tier — see docs/DEPLOY.md for the free one).
It's worth it if you'd rather own a real VM with a mounted disk than depend on Turso and R2's free
tiers, or if this ever needs to scale beyond one machine: Fly's LiteFS lets SQLite itself replicate
across regions/machines, a path Render's or Railway's disks don't offer. Until then this runs as
exactly **one machine** (SQLite is single-writer anyway, so extra replicas against the same volume
would not help).

For the free path (Render + Turso + Cloudflare R2), see `docs/DEPLOY.md` instead — it needs no
Dockerfile/`fly.toml` and no local disk at all, since `src/db.js` and `src/uploads.js` both fall
back to Turso/R2 when their env vars are set.
