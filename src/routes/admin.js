...
const { GLOBAL_KEYS, LANG_KEYS, clean } = require('../settings');
...
// is not one of them) is never touched by saving settings. The keys and the filtering live in src/settings.js
// so that scripts/seed-about.js, which writes the same rows, cannot write what this form would reject.
...
router.post('/settings', async (req, res) => {
  ...
  await transaction(async () => {
    for (const key of GLOBAL_KEYS) {
      // a value clean() rejects is silently stored as empty, never a validation error - this route has no 400 path
      await upsertSetting(key, '*', clean(key, text(body[key])));
    }
    for (const key of LANG_KEYS) {
      for (const lang of ['th', 'en']) {
        await upsertSetting(key, lang, clean(key, text((body[lang] || {})[key])));
      }
    }
  });
