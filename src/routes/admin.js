...
const GLOBAL_SETTING_KEYS = ['site_name', 'about_image', 'about_photo', 'about_colors', 'github_url', 'linkedin_url', 'x_url', 'email'];
const LANG_SETTING_KEYS = [
  'tagline', 'about_body', 'about_image_alt', 'about_name', 'about_facts',
  'about_quote', 'about_photo_caption', 'about_contact', 'privacy_body'
];
...
// about_image (the portrait) and about_photo (the picture beside the quote) become <img src> on /about.
// /admin/upload answers with '/uploads/<name>' locally and an absolute R2 URL in production, so both shapes
// are allowed and nothing else is: '//host/x' and a javascript: or data: value typed into the field by hand
// are treated as empty, the same silent handling as the URLs above.
const IMAGE_PATTERN = /^(https?:\/\/\S+|\/[^\s/]\S*)$/i;
const IMAGE_SETTING_KEYS = ['about_image', 'about_photo'];
...
router.post('/settings', async (req, res) => {
  ...
  await transaction(async () => {
    for (const key of GLOBAL_SETTING_KEYS) {
      ...
      if (IMAGE_SETTING_KEYS.includes(key) && value && !IMAGE_PATTERN.test(value)) value = '';
